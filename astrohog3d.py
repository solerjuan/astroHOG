...
# --------------------------------------------------------------------------------------------------------------------------------
def HOGcorr_cubeLITE(cube1, cube2, pxsz=1., ksz=1., mode='nearest', mask1=0., mask2=0., gradthres1=0., gradthres2=0., weights=None):
   ...
   assert cube2.shape == cube1.shape, "Dimensions of ima2 and ima1 must match"
   sz1=np.shape(cube1)
   if weights is None:
      weights=np.ones(sz1)
